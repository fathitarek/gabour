...
@section('content')
<div class="bread-crumb"><p><a >{{$current_website->display_name_en}}  Similar Sections</a> / <span>Add New Value Parts Leaflet Download </span></p></div>
    <hr/>
          ...
          @include('Admin.form_parcials.upload_field',['name'=>'pdf_en','label'=>'pdf English:'])
            <br>
             <div>{!!$record->pdf_en ? '<a target="_blank" href="/'.$current_website->name.'/images/'.$record->pdf_ar.'" height="40">'.$record->pdf_en. '</a>':''!!}</div>
        ...
        {!!Form::hidden('category','value_parts_leaflet_download',['id'=>'category']) !!}
