...
@media screen and (max-width:640px){
    ...
    .custom-tooltip-styling {
        ...
        <div id="CONTENTS_DATA">
            @if(count($page['the_benefits_ar']['special_listing_1_data']))
            <div id="GP_BENEFIT_DATA">
                @foreach($page['the_benefits_ar']['special_listing_1_data'] as $index =>$the_benefits)
                ...
                <dl id="GP_LIST_A" class="clearfix">
                    ...
                    <dd><h4>{{$the_benefits->title_ar}}</h4>
                        <p>{{$the_benefits->small_desc_ar}}</p>
                        <p class="more_info"><a href="#popup{{$index}}" class="open-popup-link">اقرا المزيد</a></p></dd>
                </dl>
                <div id="popup{{$index}}" class="gp_popup mfp-hide">
                    <h3>{{$the_benefits->title_ar}}</h3>
                    <div class="popup_img">
                        <img src="/{{$current_website->name}}/images/{{$the_benefits->image_en}}">
                    </div>
                    <div class="popup_text">
                        <p> {{$the_benefits->full_desc_ar}}</p>
                    </div>
                </div>
