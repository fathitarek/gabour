        <div id="HEADER_WRAP">
          ...
          <div id="MENU_FIELD">
            <div id="MB_MENU_HEAD" class="clearfix mb_display">
              ...
              <script src="./ar/PartsAccessories_files/search_engine_zero.js.download"></script>
              <form action="http://www.mitsubishi-fuso.com/content/fuso/en/search_result.html" name="searchform" id="searchform">
                <dl class="clearfix">
                  ...
                  <dd>
                    <input type="image" src="./ar/PartsAccessories_files/null.gif" alt="" name="searchBtn2" id="searchBtn3">
                  </dd>
                </dl>
              </form>
            ...
            <div id="MENU_WRAPS">
                <nav>
                    <ul id="GLOBALMENU_FIELD">
                        {{--{{$page['homepage_main_menu_arabic']['special_listing_1_data'][0]->menuItems}}--}}
                        @foreach($page['homepage_main_menu_arabic']['special_listing_1_data'][0]->menuItems as $index =>$menu_item)
                                        ...
                                        <li class="menu_all clearfix" id="{{$menu_item->name}}"> <a href="#" class="menu_all"> <img src="/fuso/images/menu_items/{{$menu_item->image_on}}" alt="{{$menu_item->display_name}}" class="p_gnv pc_display" data-pin-nopin="true" /> <span class="mb_icon mb_display"> <img src="./assets/mb_menu_products.png" alt="{{$menu_item->display_name}}" class="p_gnv" /></span> <span class="mb_txt mb_display">{{$menu_item->display_name}}</span></a>
                                            <ul class="submenu clearfix" style="display: none;">
                                                @foreach($menu_item->menuItems as $sub_index =>$sub_menu_item)
                                                    <li> <a href="{{$sub_menu_item->url}}" id="{{explode( '/', $sub_menu_item->url)[0]}}"> <img src="/fuso/images/menu_items/{{$sub_menu_item->image_on}}" alt="{{$sub_menu_item->display_name}}" class="p_gnv pc_display" /> <img src="./ar/assets/active_alw.png" alt="" class="active_alw pc_display" /> <span class="mb_txt mb_display">{{$sub_menu_item->display_name}}</span></a></li>
                                                @endforeach
