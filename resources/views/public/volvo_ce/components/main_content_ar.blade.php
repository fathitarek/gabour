<div class="row heroImgX2">
    <div class="col-md-10 col-md-offset-1">
        ...
        <h3>{{$page['Category'][0]['display_name_ar']}}</h3>
...
<div class="row headTitleX">
    <div class="col-md-10 col-md-offset-1">
        <h4>{{$page['Category'][0]['text_title_ar']}}</h4>
        <p>{{$page['Category'][0]['text_ar']}}</p>
    </div>
</div>
        ...
        <div class="col-md-10 col-md-offset-1">
          <div class="row">
            @foreach($page['Category'][0]->categories as $index =>$sub_category)
                <div class="col-md-4"><img src="/{{$current_website->name}}/images/categories/{{$sub_category->small_image}}" class="img-responsive" alt="{{$sub_category->display_name_ar}}">
                  <a href="/volvo_ce/category_ar?m=Category&item={{$sub_category->name}}">{{$sub_category->display_name_ar}}<span class="glyphicon glyphicon-menu-left
    "></span></a>
                  <p>{{$sub_category->note_ar}}</p>
                </div>
            @endforeach
          ...
          <div class="row">
            @foreach($page['Category'][0]->products as $index =>$product)
                <div class="col-md-4 text-left"><img src="/{{$current_website->name}}/images/product/{{$product->image}}" class="img-responsive" alt="Placeholder image">
                  <a href="/volvo_ce/product_ar?m=Product&item={{$product->name}}" class="linkLeftDark"><h5>{{$product->model}}<span class="glyphicon glyphicon-chevron-left"></span></h5></a>
